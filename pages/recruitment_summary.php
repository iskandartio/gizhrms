<?php
	...
	$res=db::DoQuery("select e.first_name, e.last_name, c.vacancy_code, c.vacancy_code2, c.vacancy_name, d.update_time
	...
	foreach ($res as $row) {
		$result.="<tr><td>".$row['contract_history_id']."</td><td>".$row['vacancy_name']." (".$row['vacancy_code']."-".$row['vacancy_code2'].")</td><td>".formatDate($row['update_time'])."</td>";
...
<script>
	...
	function Print() {		
		window.open("recruitment_summary_ajax?contract_history_id="+$(this).closest("tr").children("td:eq(0)").html(),"_blank");
	}
	function Export() {
