...
if ($type=='closing') {
	...
	$email=db::select_single('m_user','user_name','user_id=?','' array($user_id));
	...
	foreach ($res as $rs) {
		...
		$data=shared::copyToData($rs, $data, ['title','contract1_start_date','contract1_end_date'
			...
			, 'address', 'country_id', 'country_name', 'province_id', 'city_id', 'post_code', 'phone1', 'phone2', 'computer_skills', 'professional_skills','email']);
